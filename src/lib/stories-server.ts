...
import { STORY_BUCKET, type PublishedStory } from "./stories";

/** Server-side reads of approved stories only, through the public view. */

const COLUMNS = "id, created_at, realm, place, travelled_on, title, body, places, photos, author_name";
...
async function withPhotoUrls(supabase: SupabaseClient, rows: Row[]): Promise<PublishedStory[]> {
  ...
  if (paths.length === 0) return rows.map((row) => ({ ...row, photoUrls: [] }));
  ...
  return rows.map((row) => ({ ...row, photoUrls: (row.photos ?? []).map((path) => urls.get(path)).filter((u): u is string => Boolean(u)) }));
}

export async function publishedStories(options: { place?: string; realm?: "earth" | "beyond"; limit?: number } = {}): Promise<PublishedStory[]> {
